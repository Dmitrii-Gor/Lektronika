...
void set_matrix_population(int counter_life)
{
    ...
    for(int i=0; i<ROWS; i++)
    {
        for(int j=0; j<COLS; j++)
        {
            if(counter == counter_life) // огранчение на counter_life единичек в матрице
            {
                break;
            }
            matrix[i][j] = rand()%2;
...
int check_cell_neighbour(int cell_index_x, int cell_index_y)
{
   ...
   for(int i = -1; i <= 1; i++)
   {
        for(int j = -1; j<=1; j++)
        {
            ...
            counter++;
            if(counter != 5 && matrix[x][y] == 1) // ячейка которую мы передали в эту функцию находится под 5 номером -> пропускаем её
            {
               counter_alive_cell++;
            }
        }
